...
var twist = (function (elem, value) {
  ...
  var max = !isNaN(options.max) ? options.max : 40;
  var min = !isNaN(options.min) ? options.min : -10;
  if (options.direction === 'left') {
    max = -max;
    min = -min;
  }
  var twist = (max - min) * value;
  elem.style.transform = 'rotate(' + (min + twist) + 'deg) translateX(' + (max/4-twist/1.4) +'px)';
});
